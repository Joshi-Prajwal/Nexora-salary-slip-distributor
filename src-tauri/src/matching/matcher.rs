...
fn parse_filename_intelligence(file_name: &str) -> (Option<String>, Option<String>) {
    let clean = file_name.trim_end_matches(".pdf").trim_end_matches(".PDF");
    if let Some((code, name)) = clean.split_once('-') {
        let trimmed_code = code.trim();
        let trimmed_name = name.trim();
        let code_res = if !trimmed_code.is_empty() { Some(trimmed_code.to_string()) } else { None };
        let name_res = if !trimmed_name.is_empty() { Some(trimmed_name.to_string()) } else { None };
        (code_res, name_res)
    } else {
        (None, None)
    }
...
impl EmployeeMatcher for StandardMatcher {
    fn get_candidates(&self, slip: &SalarySlip, employees: &[Employee]) -> Vec<MatchCandidate> {
        ...
        let (filename_code, filename_name) = parse_filename_intelligence(&slip.file_name);
        let norm_filename_code = filename_code.as_deref().map(normalize_employee_id).unwrap_or_default();
        let norm_filename_name = filename_name.as_deref().map(normalize_string).unwrap_or_default();

        // Effective ID signal (PDF text preferred, filename code fallback)
        let effective_slip_id = if !norm_slip_id.is_empty() {
            norm_slip_id.clone()
        } else {
            norm_filename_code.clone()
        };

        // Effective Name signal (PDF text preferred, filename name fallback)
        let effective_slip_name = if !norm_slip_name.is_empty() {
            norm_slip_name.clone()
        } else {
            norm_filename_name.clone()
        };
        ...
        for emp in employees {
            ...
            // 1. Employee ID Check (Deterministic Priority 1)
            let id_matched = !effective_slip_id.is_empty()
                && !norm_emp_id.is_empty()
                && effective_slip_id == norm_emp_id;

            if id_matched {
                matched_fields.push("Employee ID".to_string());
                score += 100.0;
            } else if !effective_slip_id.is_empty() {
                unmatched_fields.push("Employee ID".to_string());
            }

            // 2. Email Check (Deterministic Priority 2/3)
            ...
            } else if !norm_slip_email.is_empty() && !norm_emp_email.is_empty() {
                unmatched_fields.push("Email".to_string());
                // Conflict flag: ID matches but Email contradicts!
                if id_matched {
                    is_conflict = true;
                }
            }

            // 3. Phone Check (Deterministic Priority 2/3)
            ...
            } else if !norm_slip_phone.is_empty() && !norm_emp_phone.is_empty() {
                unmatched_fields.push("Phone".to_string());
                // Conflict flag: ID matches but Phone contradicts!
                if id_matched {
                    is_conflict = true;
                }
            }

            // 4. Name Check (Deterministic Priority 4/5/6)
            let name_matched = !effective_slip_name.is_empty()
                && !norm_emp_name.is_empty()
                && (effective_slip_name == norm_emp_name
                    || effective_slip_name.contains(&norm_emp_name)
                    || norm_emp_name.contains(&effective_slip_name));
            ...
            } else if !effective_slip_name.is_empty() {
                unmatched_fields.push("Name".to_string());
            }

            if is_conflict {
                score = 50.0; // Penalty score for conflicting identifiers
            }
            ...
            if score > 0.0 {
                let explanation = if is_conflict {
                    format!("Conflict detected: ID matches but {} contradicts.", unmatched_fields.join(", "))
                } else {
                    format!("Matched fields: {}. Score: {}%.", matched_fields.join(", "), score as u32)
                };
    ...
    fn match_slip(&self, slip: &SalarySlip, employees: &[Employee]) -> MatchResult {
        ...
        } else if best_candidate.score >= 85.0 {
...
mod tests {
    ...
    fn create_test_slip(id: &str, file_name: &str, emp_code: Option<&str>, name: Option<&str>, email: Option<&str>, phone: Option<&str>) -> SalarySlip {
        SalarySlip {
            id: id.to_string(),
    ...
    fn test_exact_employee_id_matching() {
        ...
        let slip = create_test_slip("slip-1", "EMP1024.pdf", Some("EMP1024"), None, None, None);
    ...
    #[test]
    fn test_filename_intelligence_matching() {
        let emp = create_test_employee("emp-2", "203", "Dr L B Singh", "singh@example.com", "+91 9876543210");
        let slip = create_test_slip("slip-2", "203-Dr L B Singh.pdf", None, None, None, None);
        let matcher = StandardMatcher::new();
    ...
    #[test]
    fn test_conflicting_phone_detection() {
        let emp = create_test_employee("emp-3", "111", "Ashwini Kulkarni", "ashwini@example.com", "+91 8888888888");
        // PDF has ID 111 but conflicting phone +91 9999999999
        let slip = create_test_slip("slip-3", "111-Ashwini.pdf", Some("111"), Some("Ashwini Kulkarni"), None, Some("+91 9999999999"));
        let matcher = StandardMatcher::new();
    ...
    fn test_reconciliation_invariant() {
        let emps = vec![
            create_test_employee("emp-1", "101", "Alice", "alice@example.com", "111"),
            create_test_employee("emp-2", "102", "Bob", "bob@example.com", "222"),
        ];

        let slips = vec![
            create_test_slip("s-1", "101.pdf", Some("101"), None, None, None), // Exact
            create_test_slip("s-2", "102.pdf", Some("102"), None, None, Some("999")), // Conflict (phone differs)
            create_test_slip("s-3", "unknown.pdf", None, None, None, None), // No Match
        ];
        ...
        for slip in &slips {
            let res = matcher.match_slip(slip, &emps);
            match res.status.as_str() {
                "EXACT_MATCH" => exact += 1,
                "STRONG_MATCH" => strong += 1,
                "POSSIBLE_MATCH" => possible += 1,
                "CONFLICT" => conflict += 1,
                "NO_MATCH" => no_match += 1,
                _ => unresolved += 1,
            }
        ...
        let total = slips.len();
        // Authoritative reconciliation invariant assertion
        assert_eq!(exact + strong + possible + conflict + no_match + unresolved, total);
        assert_eq!(exact, 1);
